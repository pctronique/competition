          ...
          if (modif) {
            ...
            if (!isNaN(score)) {
              ...
              if (scoreInt >= 0) {
                ...
                if (myIndex != -1) {
                  addGameUser(
                    gameUserTab[myIndex].gameId,
                    gameUserTab[myIndex].userId,
                    scoreInt
                  );
                  saveLocalSGBD();
                }
  ...
  document
    ...
    .forEach((element) => {
      element.addEventListener("click", function (e) {
        let id = parseInt(
          document
            .getElementById(this.parentNode.parentNode.id)
            .id.split("_")[1]
        );
        let name = "";
...
function addRowUserGame(gameUser) {
  ...
  let type_input = "tab_input_end";
  if (validate == 1) {
    type_input = "tab_input_start";
  } else if (validate == 2) {
    type_input = "tab_input";
  }
  return (
    '<tr id="concours_' +
    gameUser.id +
  ...
  if (nbLigne > 0) {
    ...
    document
      ...
      .forEach((element) => {
        ...
        let idGameUser = parseInt(element.id.split('_')[1]);
        let myIndexGameUser = recupId(gameUserTab, idGameUser);
        if(myIndexGameUser != -1) {
          let myIndexGame = recupId(gameTab, gameUserTab[myIndexGameUser].gameId);
          if(myIndexGame != -1) {
            elementTd.innerHTML = addPopoverInfo(gameTab[myIndexGame], validate);
          }
        }
      });
      activePopover();
...
function addEventAllScore() {
  document
    .getElementById("participer")
    .addEventListener("click", addParticipation);
